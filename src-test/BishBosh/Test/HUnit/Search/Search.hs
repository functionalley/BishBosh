...
evaluationOptions	= Input.EvaluationOptions.mkEvaluationOptions Data.Default.def {-rankValues-} Data.Default.def {
	Input.CriteriaWeights.getWeightOfMobility	= Attribute.CriterionWeight.mkCriterionWeight $ recip 100
} False Nothing
...
testCases	= Test.HUnit.test $ map (
	\(searchDepth, fen, moveOptions) -> let
		turnString	= Notation.MoveNotation.showNotation Data.Default.def {-Smith-} $ case Search.Search.getQuantifiedGames $ Control.Monad.Reader.runReader (
			Search.Search.search searchDepth $ Search.SearchState.initialise (
				...
				) searchOptions Data.Default.def {-Zobrist-} Property.Empty.empty {-MoveFrequency-} (
					Property.ForsythEdwards.readFEN fen	-- Game.
				) :: Evaluation.PositionHashQuantifiedGameTree.PositionHashQuantifiedGameTree T.X T.Y T.PositionHash T.CriterionValue T.WeightedMean
			 )
		 ) searchOptions of
...
--		FEN									Accepted moves	Observed move, at various search-depths.
--													4		5
	...
	(4,	"3rr3/2pq2pk/p2p1pnp/8/2QBPP2/1P6/P5PP/4RRK1 b - -",			"e8e4p"),	-- c7c5.
